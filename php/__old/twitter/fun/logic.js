...
function total_number(){
		...
		function(data){
				$.each(data, function(key, value){
						...
						$('#number').append(renderNumber(value));
				});
		});
...
function renderNumber(value){
		if($('.number').length>0){ //If we have more than nine tweets
				$('.number:first').toggle(200);//remove it form the screen
				$('.number:first');//and it's class
				$(".number").remove(); //sweeps the already hidden elements
		}
		return '<div class="number">total tweets so far is '+value+'</div>';
}
